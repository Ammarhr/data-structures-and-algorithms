...
const count = (target, input) => {
    ...
    let overall = input.reduce((overall, newArr) => {
        if (newArr === []) {
            return 0;
        }
        let secCounter = newArr.reduce((secCounter, value) => {
            if (target === value) {
                secCounter = secCounter + 1;
            }
            return secCounter;
        }, 0);
        overall = overall + secCounter;
        return overall;
    }, 0);
    return overall;
};
...
const totalSum = (input) => {
    // Solution code here...
    let overAll = input.reduce((sum, newArr) => {
        let secSummer = newArr.reduce((subsum, value) => {
            subsum += value;
            return subsum;
        }, 0);
        sum += secSummer;
