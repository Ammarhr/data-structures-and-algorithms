...
const notInFirstArray = (forbiddenValues, arr) => {
    // Solution code here...
    let numArray = arr.filter(uniqueNumber => {
        if (!forbiddenValues.includes(uniqueNumber)) {
            return uniqueNumber;
        }
    });
    return numArray;
...
const getStatName = (arr, minBaseStat) => {
    ...
    let objArr = [];
    arr.forEach(objVal => {
        let name = objVal.stat.name;
        if (objVal.baseStat > minBaseStat) {
            objArr.push(name);
        }
    });
    return objArr;
