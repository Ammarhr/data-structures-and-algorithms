...
const getCourseKeys = (obj) => {
    // Solution code here...
    const resultKey = Object.keys(obj);
    return resultKey;
};
...
const getHouses = (arr) => {
    ...
    // Solution code here...
    arr.forEach((objects) => {
        Object.entries(objects).forEach((entries) => {
            entries.forEach((keys, i) => {
                if (keys === 'house') {
                    let value = i + 1;
                    houses.push(entries[value]);
                }
            });
        });
    });
...
const hasChildrenValues = (arr, character) => {
    ...
    arr.forEach((person) => {
        if (person.name === character) {
            if (Object.values(person)[0].length > 0) {
                hasChild = true;
            }
        }
    });
